...
--- *The following three sections are not functional, but have been retained to show how raw data was manipulated. Will be updated to include raw data manipulations following publication* ---
 ...
 * Source: David N. Chapter 6—Selection of Variables and Factor Derivation. Commercial data mining: Processing, analysis and modeling for predictive analytics projects. 2014:79-104.
...
ggplot(. , aes(x = predicted_diff, y = as.factor(ExpYrNum), color = Treatment_Combo)) +
  ...
  labs(
   # title = "Model Predictions for Phase Start and End by Year",
    x = "Treatment response (Mean Day of Year ± 1 SE)",
...
Zero-Inflated Negative Binomial	| Excess zeros + overdispersion   | glmmTMB() (family = nbinom2)
 ...
 # *Some notes on model distribution and selection with AICc:*
     # If AICc < 2 units difference → Choose the simpler model for both datasets.
     # If AICc > 2 units difference → Stick with the model that fits each dataset better individually.
...
This workflow (above) rigorously confirms that the GLMM approach with Poisson and Gamma distributions is the most suitable for analyzing our phenological data. This methodology properly handles count data, non-normal distributions, and overdispersion while balancing complexity and model fit.
    ...
    A note about getting DF from glmers: this is basically impossible (there is no method for it)
        ...
        + From Ben Bolker: Run the model in lme (if possible) and use the denominator df reported there (which follow a simple ‘inner-outer’ rule which should correspond to the canonical answer for simple/orthogonal designs), applied to t or F tests
...
ggplot(., aes(y = Treatment_Combo, x = predicted, color = Treatment_Combo, linetype = Treatment_Combo)) +
  ...
  labs(
    x = expression(bold("Start of Leaves") ~ "- Mean Day of Year ± 1 SE"),
...
ggplot(., aes(y = Treatment_Combo, x = predicted, color = Treatment_Combo, linetype = Treatment_Combo)) +
  ...
  geom_errorbar(aes(xmin = predicted - link_se, xmax = predicted + link_se), position = position_dodge(0.5), width = 0.1, size = 1) +
  labs(
    x = expression(bold("Start of Open Flowers") ~ "- Mean Day of Year ± 1 SE"),
    y = expression(bold("Treatment"))
  ) +
  custom_theme +
...
ggplot(., aes(y = Treatment_Combo, x = predicted, color = Treatment_Combo, linetype = Treatment_Combo)) +
  ...
  geom_errorbar(aes(xmin = predicted - link_se, xmax = predicted + link_se), position = position_dodge(0.5), width = 0.1, size = 1) +
  labs(
    x = expression(bold("Start of Ripe Fruits") ~ "- Mean Day of Year ± 1 SE"),
    y = expression(bold("Treatment"))
  ) +
  custom_theme +
...
ggplot(., aes(y = Treatment_Combo, x = predicted, color = Treatment_Combo, linetype = Treatment_Combo)) +
  ...
  geom_errorbar(aes(xmin = predicted - link_se, xmax = predicted + link_se), position = position_dodge(0.5), width = 0.1, size = 1) +
  labs(
    x = expression(bold("End of Leaf Senescence") ~ "- Mean Day of Year ± 1 SE"),
    y = expression(bold("Treatment"))
  ) +
  custom_theme +
...
ggplot(newdata_all_models %>%
  ...
  labs(
    x = expression(bold("End of Open Flowers") ~ "- Mean Day of Year ± 1 SE"),
    y = expression(bold("Treatment"))
  ) +
  custom_theme +
  scale_color_manual(
...
ggplot(., aes(y = Treatment_Combo, x = predicted, color = Treatment_Combo, linetype = Treatment_Combo)) +
  ...
  geom_errorbar(aes(xmin = predicted - link_se, xmax = predicted + link_se), position = position_dodge(0.5), width = 0.1, size = 1) +
  labs(
    x = expression(bold("End of Ripe Fruits") ~ "- Mean Day of Year ± 1 SE"),
    y = expression(bold("Treatment"))
  ) +
  custom_theme +
